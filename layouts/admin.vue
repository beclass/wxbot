<template>
  <a-layout class="layout-admin">
    <a-layout-sider :trigger="null" collapsible v-model="collapsed" breakpoint="lg" class="layout-sider">
      ...
      <a-layout-header class="layout-header">
        ...
        <div class="auth-actions">
            <a-dropdown placement="bottomRight">
              <span>
                {{this.$auth.user.username}}<a-icon type="down" />
              </span>
              <a-menu slot="overlay">
                <a-menu-item>
...
export default Vue.extend({
  ...
  computed: {
    title(){
      return this.collapsed ? this.$store.state.robot.appName.substr(0,1) :this.$store.state.robot.appName;
    },
    marginLeft () {
